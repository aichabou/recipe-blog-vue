<template>
    <header class="l-header">
      <nav class="nav bd-grid">
        <div class="nav__menu" id="nav-menu">
          <ul class="nav__list">
            <img :src="require('@/assets/image/logo.png')" alt="Logo du blog" class="logo">
            <li class="nav__item"><a href="/welcome" class="nav__link ">Accueil</a></li>
            <li class="nav__item"><a href="/home" class="nav__link">Recettes</a></li>
            <li class="nav__item">
              <router-link to="/welcome" class="nav__link" id="logout-link" @click="deconnecter">Se déconnecter</router-link>
            </li>
          </ul>
        </div>
        <div class="nav__toggle" id="nav-toggle">
          <i class='bx bx-menu'></i>
        </div>
      </nav>
    </header>
  </template>
  
  <script>
  export default {
    name: 'AppHeaderLogout',
    methods: {
      deconnecter() {
        // Supprimez les informations d'authentification (exemple : token JWT)
        localStorage.removeItem('token'); // Supprimez le token JWT du stockage local
  
        // Redirigez l'utilisateur vers la page d'accueil
        this.$router.push('/welcome');
      }
    }
  }
  </script>
